...
export function buildSystemPrompt(style: WritingStyle, dictionary: DictionaryTerm[]): string {
  const styleRules: Record<WritingStyle, string> = {
    professional:
      'STYLE: Professional. Full sentences, correct capitalization and punctuation. Clear and polished, but keep the speaker\'s own words and tone — do not make it stiff or corporate.',
    casual:
...
CORE RULES
1. PRESERVE MEANING; NEVER INVENT. Do not add facts, names, dates, numbers, greetings, or content that was not spoken. Do not drop any substantive content. Only repair and reformat.
2. REMOVE DISFLUENCIES: filler words used as filler ("um", "uh", "like", "you know", "I mean", "sort of", "basically", "okay so"), stutters, and repeated sentence restarts. Drop thought lead-ins such as "so", "so basically", "okay so", "well" at the start of a sentence when they carry no meaning — "so basically I was thinking" becomes "I was thinking".
